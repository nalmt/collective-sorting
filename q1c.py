...
class Agent:
    ...
    def get_number_of_around_depot(self, letter):
        return self.t.count(letter)

    # return number of the object 'letter' ('A' or 'B')
    # in the boxes around (N, S, E, W)
    def get_number_of_around_prise(self, letter):
        number_around = 0
    ...
    def action(self):
        ...
        if self.objet_porte == '0':
            if encountered_object != '0':
                proba_prise = self.p_prise(self.fprise(encountered_object))
            ...
            if encountered_object == '0':
                proba_depot = self.p_depot(self.fdepot(self.objet_porte))
    ...
    def fprise(self, encountered_object):
        return self.get_number_of_around_prise(encountered_object)/self.number_of_boxes()

    def fdepot(self, encountered_object):
        return self.get_number_of_around_depot(encountered_object)/len(self.t)
